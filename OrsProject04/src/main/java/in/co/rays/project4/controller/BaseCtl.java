...
public abstract class BaseCtl extends HttpServlet{
    ...
    protected BaseBean populateDTO(BaseBean dto, HttpServletRequest request) {
        ...
        dto.setModified_By(modifiedBy);

        int cdt = DataUtility.getInt(request.getParameter("createdDatetime"));

        if (cdt > 0) {
            dto.setCreated_Date_Time(DataUtility.getTimestamp(cdt));
        } else {
            dto.setCreated_Date_Time(DataUtility.getCurrentTimestamp());
        }

        dto.setModified_Date_Time(DataUtility.getCurrentTimestamp());
