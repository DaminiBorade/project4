...
public class MarksheetModel {
	...
	public int add(MarksheetBean bean) throws ApplicationException, DuplicateException {
		...
		StudentBean studentbean = sModel.findByPK(bean.getStudent_Id());
		 bean.setName(studentbean.getFrist_Name() + " "
		 + studentbean.getLast_Name());

		MarksheetBean duplicateMarksheet = findByRollNo(bean.getRoll_No());
		int pk = 0;
		...
		try {
			...
			pstmt.setInt(3, bean.getStudent_Id());
	...
	public MarksheetBean findByRollNo(String rollNo) throws ApplicationException {
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setStudent_Id(rs.getInt(3));
				bean.setName(rs.getString(4));
				bean.setPhysics(rs.getInt(5));
				bean.setChemistry(rs.getInt(6));
				bean.setMaths(rs.getInt(7));
				bean.setCreated_By(rs.getString(8));
				bean.setModified_By(rs.getString(9));
				bean.setCreated_Date_Time(rs.getTimestamp(10));
				bean.setModified_Date_Time(rs.getTimestamp(11));
			}
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public MarksheetBean findByPK(int pk) throws ApplicationException {
	...
	public void update(MarksheetBean bean) throws ApplicationException, DuplicateException {
		...
		Connection conn = null;
		MarksheetBean beanExist = findByRollNo(bean.getRoll_No());
		...
		StudentBean studentbean = Model.findByPK(bean.getStudent_Id());// method
																		// create
																		// in
																		// student
																		// bean
																		// check
		bean.setName(studentbean.getFrist_Name() + " " + studentbean.getLast_Name());
		...
		try {
			...
			pstmt.setInt(2, bean.getStudent_Id());
			...
			pstmt.setTimestamp(9, bean.getCreated_Date_Time());
			pstmt.setTimestamp(10, bean.getModified_Date_Time());
	...
	public List search(MarksheetBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getRoll_No() != null && bean.getRoll_No().length() > 0) {
				sql.append(" AND roll_no like '" + bean.getRoll_No() + "%'");
			}
